...
async def _resolve_binding(request: Request, project_id: str) -> BranchBinding:
    ...
    try:
        async with sessionmaker() as session:
            return await BranchBindingStore(session).resolve_active(project_id)
...
async def get_briefing(
    ...
    binding = await _resolve_binding(request, project_id)
    try:
...
@router.get("/briefing/status", response_model=FrontendBriefingStatusResponse)
async def get_briefing_status(
    ...
    try:
        response = await run_in_threadpool(
            _vss_client(request).briefing_status,
            binding.vss_project_id,
        )
    except VssIntegrationError as exc:
        _raise_vss_error(exc)
    return to_frontend_briefing_status(
        response,
        frontend_project_id=project_id.strip(),
        vss_project_id=binding.vss_project_id,
    )
