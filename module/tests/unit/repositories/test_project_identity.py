...
def test_canonical_vss_project_id_uses_repo_at_branch_once() -> None:
    assert (
        ...
        == "vss_server@test-merge--module"
    )
    assert (
        canonical_vss_project_id("h5vision/vss_server", "refs/heads/module")
        == "vss_server@module--module"
    )
...
def test_canonical_vss_project_id_makes_slash_branch_collision_safe() -> None:
    ...
    assert project_id.startswith("vss_server@feature-login-") and project_id.endswith("--module")
