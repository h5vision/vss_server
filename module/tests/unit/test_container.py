...
def test_build_container_with_defaults(tmp_path):
    ...
    assert container.collected_snapshot_publisher is not None
    assert not hasattr(container.collected_snapshot_publisher, "_vss_client")
    assert container.repository_collection_service is not None
    assert container.commit_catalog_service is not None
    assert container.snapshot_index_service is not None
