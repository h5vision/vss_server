...
**작성일시**: 2026-08-27 KST (Phase 3B-1 로컬 런타임 연결 반영)
...
**테스트**: Contract 39 / Unit 44 / Integration 7, 총 90개 통과. Ruff 오류 0건.
compileall 성공. PostgreSQL offline migration SQL 생성 성공.

현재 FastAPI는 liveness/readiness와 Frontend `/v1/projects`, `/v1/models`,
`/v1/briefing` 조회 proxy를 제공합니다. Frontend overlay, index status 동기화, Admin
CRUD와 materialization route는 아직 등록되지 않았으며 로컬 fake integration을 실환경
E2E 완료로 해석하지 않습니다.
