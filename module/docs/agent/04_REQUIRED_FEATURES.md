...
- 등록 Repository는 Tracked Branch별 `/home/ubuntu/repos/.snapshot-worktrees/<repo-basename>/<repository-id>/branches/<safe-branch-component>` working copy와 내부 bare object cache로 관리합니다.
...
- VSS `/index.project_root`는 current tracked HEAD일 때 해당 branch-scoped working copy를 사용하고, historical/inactive Snapshot은 검증된 immutable Snapshot 경로를 사용합니다.
