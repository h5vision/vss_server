...
다음       Phase 3A-3 포트 4180 Admin API·인증/RBAC/UI
...
ingress에 공개하지 않습니다. Phase 3A-2 수집 코어는 app 내부 service이며 아직
`/v1/admin/*` route나 scheduler로 노출하지 않습니다.
...
Windows 전체 130 passed + POSIX 1 skipped
...
다음 구현은 Phase 3A-3입니다. 독립 Admin service의 포트는 `4180`으로 고정하고 정적 UI와
Backend loopback BFF를 직접 제공합니다. Nginx는 기본 구성에 넣지 않으며 인증/RBAC,
감사 actor와 AWS 접근 범위를 먼저 확정합니다. Phase 3A-4 Webhook은 공개 HTTPS,
...
RBAC, Admin Web 빌드 위치와 `4180` 접근/TLS 경계 확정 전에는 외부 mutation을 공개하지
않습니다.
Frontend의
