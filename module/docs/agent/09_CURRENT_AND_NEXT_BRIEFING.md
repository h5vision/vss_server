...
다음 구현  Phase 5 상태 동기화·재시작 복구·재시도
...
아직 노출하지 않는 주요 API는 `/v1/index/status`와 `/v1/admin/*`입니다. 전자는 Phase 5의
Snapshot/VSS 상태 동기화가 필요하고, 후자는 IdP/RBAC와 독립 Admin Web 배포 경계가
확정돼야 합니다.
...
module 원격 기준 SHA   2a4f1427d4e0d0f9396f33e50532362f36d57b6e
...
5. VSS `done`과 `index.commit == target_revision` 완료 동기화
6. accepted/indexing 상태의 Backend 재시작 복구
7. 같은 Snapshot의 안전한 수동 재시도와 attempt 증가
8. 인증된 Snapshot 이력/Admin CRUD/UI
9. retention, orphan staging/revision 정리와 용량 제한
...
- `failed|aborted`의 안전한 error/incomplete 요약 보존
...
- 복구 동시 실행을 막는 DB claim/lease 또는 동등한 단일 실행 경계 추가
- process restart integration test 추가
...
Phase 5 로컬 검증 뒤 실제 PostgreSQL·VSS·shared path로 Phase 3B-2/6 E2E를 수행합니다.
