...
### 평가기가 실제로 하는 일 — 발표에서 반드시 나올 질문

**`python -m vss.eval run` 은 LLM 을 부르지 않는다.** 모드가 둘인데 둘 다 검색까지만 한다(`vss/eval/runner.py`).

| 모드 | 하는 일 | 나오는 수치 |
|---|---|---|
| `retrieval` | 벡터(+BM25 RRF) 순위만. threshold 미적용 | Hit@k, MRR |
| `pipeline` | `search()` 전체. threshold 적용 | has_evidence, 근거 없음 recall, 최종 contexts 기준 Hit@k |

그래서 **답변 내용이 맞는지는 재지 않는다.** 재는 것은 "정답이 든 청크가 위로 올라오는가" 와
"근거 없음 판정이 제대로 걸리는가" 둘이다. 답변 품질, 인용의 충분성, 모든 근거의 회수는 별도 평가가 필요하다.

LLM 을 안 부르니 빠르다. 9/9 run(`20260909T071934Z-bc6c8e`)은 61문항 × 8칸 × 2모드를 **71초**에 끝냈다.
칸당 retrieval 3.5초 + pipeline 4.9초다. 새 문제지 7개를 다 돌려도 질의 744회라 몇 분이면 된다.
긴 것은 측정이 아니라 답지를 지운 뒤의 재인덱싱이다.

**그리고 평가기는 이 문서가 재는 것을 재 주지 않는다.** `validate_matrix` 가 보는 것은 matrix 구조와
인덱스가 저장소에 있는지까지다. gold 가 그 인덱스 안에 실제로 들어갔는지는 안 본다. run 이 남기는
문항별 기록에도 순위만 있고 "애초에 코퍼스에 있었나" 는 없다. 그래서 수집 쪽은 `suite_coverage.py` 로
따로 세서 run 옆에 적어야 한다.
...
## 7. `adocs/test/` 시험지 8장의 현황 (2026-09-12)

다음 회차의 입력이다. 원본은 레포마다 150문항(답 120 + 답없음 30)으로 생성됐는데,
**질문 문장이 25~32종류뿐이고 한 문장이 자리 20개 안팎을 가리킨다.** 같은 질문에 정답이 스무 군데면
검색기가 무엇을 맞혀야 하는지 정의가 안 된다. 표본이 작은 문제가 아니라 채점 불가다.
그래서 자리마다 질문을 하나씩 다시 쓰는 작업이 필요하다.

| 시험지 | 답 문항 | 자리 | 서로 다른 질문 | 최대 반복 | gold 상태 | 레포 자리 수 | 상태 |
|---|---:|---:|---:|---:|---|---:|---|
| `fastapi_cli.md` | 42 | 42 | 42 | 1 | 정상 | 43 | **완료** (9/11, 72문항) |
| `flask-realworld-example-app.md` | 88 | 88 | 88 | 1 | 정상 | 107 | **완료** (9/12, 118문항) |
| `sqlalchemy.md` | 120 | 120 | 28 | 19 | 정상 | 13,191 | 질문만 다시 쓰면 됨 |
| `flask-restplus-server-example.md` | 120 | 120 | 32 | 18 | 정상 | 290 | 질문만 다시 쓰면 됨 |
| `asyncer.md` | 118 | 76 | 25 | 22 | 심볼 4개 틀림 | 78 | 질문 + 심볼 4개 |
| `fastapi_new.md` | 116 | 23 | 29 | 22 | 심볼 5개 틀림 | 22 | 레포 자리가 22개뿐 |
| `cli.md` | 117 | 117 | 32 | 20 | **파일 117개 전부 없음** | — | 내용이 httpie 다. 레포가 없음 |
| `vision.md` | 120 | 120 | 28 | 24 | **파일 120개 전부 없음** | 0 | gold 가 `out/**` 빌드 산출물 |

"자리" 는 서로 다른 (파일, 심볼) 쌍이고 "레포 자리 수" 는 `ast` 로 센 함수·클래스 수다(테스트 경로 제외).
완료된 두 장은 정리 뒤의 값이다.

**문항 수의 상한은 시험지가 아니라 레포에 있다.** 한 자리를 여러 문항이 가리키면 겉보기 n 은 커도
독립 판정은 자리 수만큼이고, 한 자리를 놓칠 때 점수가 덩어리로 움직인다. `fastapi_new.md` 가
116문항으로 자리 23개를 가리켜 자리 하나가 4.3%p 를 움직이는 것이 그 예다.

`vision` 은 소스가 TypeScript 39개(`vision/vision/src/**`)고 시험지가 가리키는 `out/` 은 레포에 없다.
`cli.md` 는 gold 가 `extras/packaging/linux/build.py` 같은 httpie 경로라 어느 레포에도 없다.
둘은 고치는 것이 아니라 새로 만들어야 한다.
